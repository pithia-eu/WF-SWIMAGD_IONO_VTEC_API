...
@app.get("/", summary="Get the status of the NOA Workflow API.", description="Returns the status of the NOA Workflow API.", tags=["Execute"])
...
@app.get("/download_kp_data/", response_class=StreamingResponse)
async def download_kp_data(start_date: str, end_date: str):
    # Validate the date range
...
@app.get("/download_bmag_data/", response_class=StreamingResponse)
async def download_bmag_data(start_datetime: str = Query(...), end_datetime: str = Query(...)):
    # Validate the datetime range
...
@app.get("/download_sao_metadata_zip/", response_class=StreamingResponse)
async def download_sao_metadata_zip(start_datetime: str = Query(...), end_datetime: str = Query(...), stations: str = Query(...), characteristics: str = Query(...)):
    # Validate the datetime range
    if not validate_datetimes(start_datetime, end_datetime):
        raise HTTPException(status_code=400, detail="Invalid datetime range. Ensure the start datetime is before the end datetime and the format is YYYY-MM-DDTHH:MM:SS.")
